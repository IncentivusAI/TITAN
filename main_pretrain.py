# Copyright (c) Incentivus and affiliates.
# All rights reserved.
#
...
def main(args):
    ...
    assert (
        ...
        "gradient_accumulation * batch_size * world_size must be equal to "
        "total_batch_size"
    )
    ...
    if args.continue_from is not None and os.path.exists(args.continue_from):
        ...
        if len(checkpoint_path_list) > 0:
            ...
            args.continue_from = os.path.join(
                args.continue_from, f"model_{beginning_step}"
            )
    ...
    if args.continue_from is not None:
        ...
        if os.path.exists(training_state_path):
            logger.info(
                "Loading training state like global_step, update_step, and tokens_seen "
                f"from {args.continue_from}"
            )
            ...
            logger.warning(
                f"Did not find training state in {args.continue_from}, "
                "global step will start from zero"
            )
        logger.info("*" * 40)
    ...
    run_config.update(
        {
            "max_lr": run_config.pop("lr"),  # rename lr to max_lr to avoid conflicts
    ...
    if "galore" in args.optimizer.lower():
        ...
        for module_name, module in model.named_modules():
            if not (
                ...
                or isinstance(module, QScaleLinear)
                or isinstance(module, QLinear)
            ):
                continue
    ...
    if args.simulation:
        ...
        num_train_params = sum(p.numel() for p in trainable_params) + sum(
            p.numel() for p in trainable_params_int8
        )

    logger.info(f"Trainable params: {num_train_params / 1_000_000:.2f}M")
    if "q_galore" in args.optimizer.lower():
        logger.info(
            "Trainable params with Q-GaLore enabled: "
            f"{sum(p.numel() for p in trainable_params_int8) / 1_000_000:.2f}M"
        )
    elif "galore" in args.optimizer.lower():
        logger.info(
            "Total params with GaLore enabled: "
            f"{sum(p.numel() for p in lowrank_params) / 1_000_000:.2f}M"
        )
    elif "q_apollo" in args.optimizer.lower():
        logger.info(
            "Trainable params with Q-APOLLO enabled: "
            f"{sum(p.numel() for p in trainable_params_int8) / 1_000_000:.2f}M"
        )
    elif "apollo" in args.optimizer.lower():
        logger.info(
            "Total params with APOLLO enabled: "
            f"{sum(p.numel() for p in lowrank_params) / 1_000_000:.2f}M"
        )

    logger.info(
        f"Saving model to {args.save_dir} every {args.save_every} update steps"
    )
    ...
    for batch_idx, batch in enumerate(dataloader):
        if update_step != 0 and batch_idx < args.gradient_accumulation * update_step:
            # skipping learned data when resuming from checkpointing
            continue
        ...
        if update_step > args.num_training_steps:
            logger.info(
                f"Reached max number of update steps (f{args.num_training_steps}). "
                "Stopping training."
            )
        ...
        if global_rank == 0:
            pbar.update(1)
        # layer-wise update is done during backward; requires gradient_accumulation == 1
        ...
        if (
            ...
            logger.info(
                "Saving model and optimizer to "
                f"{current_model_directory}, update step {update_step}"
            )
            os.makedirs(args.save_dir, exist_ok=True)
            model.module.save_pretrained(
                current_model_directory, max_shard_size="100GB", from_pt=True
            )
            saving_model_weight(
                model.module, f"{current_model_directory}/pytorch_model.bin", args
            )
            ...
            with open(
                f"{current_model_directory}/training_state.json", "w"
            ) as f:
    ...
    if global_rank == 0 and not os.path.exists(current_model_directory):
        logger.info(
            f"Saving model and optimizer to {current_model_directory}, "
