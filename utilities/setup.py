...
from titus_torch import GaLoreAdamW, GaLoreAdamW8bit, GaLoreAdafactor, TITANAdamW, QTITANAdamW
...
def set_seed(args):
    ...
    # Favor determinism over autotuned speed in cuDNN
...
def setup_model(args):
    ...
    if args.weight_quant:
        ...
        logger.info("--" * 20)
        logger.info("Prepared model for Int8 training")
        logger.info("--" * 20)
...
def setup_optimization(args, model, trainable_params, param_groups, id_lowrank_params):
    ...
    if args.optimizer.lower() == "adam":
    ...
    elif args.optimizer.lower() == "titan_adamw":
        optimizer = TITANAdamW(param_groups, lr=args.lr, weight_decay=args.weight_decay, scale_front=args.scale_front)

    elif args.optimizer.lower() == "q_titan":
        optimizer = QTITANAdamW(
            param_groups,
            lr=args.lr,
        ...
        # NOTE: scheduler appears to step twice per update; double step counts as a workaround.
        optimizer_dict = {}
        ...
        # attach hook to each trainable parameter
        for p in model.parameters():
            if p.requires_grad:
    ...
    elif args.optimizer.lower() == "q_titan_per_layer":
        optimizer_dict = {}
        for p in model.parameters():
            if id(p) in id_lowrank_params:
                optimizer_dict[p] = QTITANAdamW(
                    [
                        {
                            "params": [p],
                            "rank": args.rank,
                            "update_proj_gap": args.update_proj_gap,
                            "scale": args.titan_scale,
                            "proj_type": args.proj_type,
                            "proj": args.proj,
    ...
    if not layer_wise_flag:
        ...
        # return the dict variants for per-parameter scheduling
