...
def _quantize_tensor_int8(w, q_group_size: int = -1, n_bit: int = 8):
    """
    Per-row affine quantization (uint8) with optional grouping.
...
class W8Linear(torch.autograd.Function):
    ...
    def forward(ctx, x, weight, bias):
        ...
        return forward_w_float_weight(weight, x, bias)
    ...
    def backward(ctx, grad_output):
        ...
        def backward_w_float_weight(weight, grad_output):
            ...
            (float_weight.sub_(weight.zeros)).mul_(weight.scales)
            float_weight = float_weight.reshape(weight.shape)
            return grad_output @ float_weight
        ...
        # accumulate gradient in a float buffer
        if not hasattr(weight, "float_grad"):
            weight.__setattr__("float_grad", None)

        grad_mat = grad_output.reshape(-1, out_features).t() @ x.reshape(-1, in_features)
        if weight.float_grad is not None:
            weight.float_grad += grad_mat
        else:
            weight.float_grad = grad_mat
...
class QScaleLinear(nn.Module):
    def __init__(
        ...
        # Store packed uint8 weights; gradients are tracked via a float buffer (float_grad)
        self.weight = Parameter(int8_weight, requires_grad=False).to(device)
    ...
    def forward(self, input: Tensor) -> Tensor:
        return W8Linear.apply(input, self.weight, self.bias)


def prepare_model_for_int8_training(model, args, target_module):
    """
    Replace selected nn.Linear layers with QScaleLinear (uint8-weight) layers.
    """
